...
foreach($result as $row){
    $status = '';
     ...
     if($user_status['logged_in']&&$user_status['status']==1)
     {
      $status = '<span class="label label-success">Ready</span>';
     }
     elseif($user_status['logged_in']&&$user_status['status']==0)
     {
      $status = '<span class="label label-danger">Waiting</span>';
     }
     elseif($user_status['logged_in']&&$user_status['status']==2)
     {
     ...
     <td >'.$row['email'].'</td>
     <td >'.$status.'</td>';
     if($_SESSION['user_id']==$row['user_id']&&isset($_SESSION["p_id"])==false){
         $output .= '  
