...
void Hands::checkFaceHands(Card* cards){
    int num=7;
    int endPos=0;
    bool found=0;
...
void Hands::setHands(Card* cards){
    cout << "test1" << endl;
    int num=7;
    fiveCards=new Card[5];
    int index=0;
    int scan=0;
    int found=0;
    bool moreThanFiveCards=0;
    bool Flush=0;
    int isStraight=0;
    if(num>5) moreThanFiveCards=1;
    cout << "test2" << endl;
    checkFaceHands(cards);
    isFlush(cards);
...
void Hands::displayFiveCards(){
    for(int i=0;i<5;i++){
        if(fiveCards->getFace()!=-1) {
            fiveCards->displayCard();
            fiveCards++;
        }
    }
}

void Hands::setPoints(){
    points=0;
